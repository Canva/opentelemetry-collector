...
type ServerConfig struct {
	...
	// RateLimit for this receiver (Canva addition)
...
func (sc *ServerConfig) ToServer(ctx context.Context, host component.Host, settings component.TelemetrySettings, handler http.Handler, opts ...ToServerOption) (*http.Server, error) {
	...
	// The RateLimit interceptor should always be right after auth (Canva addition)
	if sc.RateLimit.HasValue() {
		rateLimit := sc.RateLimit.Get()
		limiter, err := rateLimit.rateLimiter(host.GetExtensions())
		if err != nil {
			return nil, err
		}
